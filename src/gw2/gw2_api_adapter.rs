...
use crate::gw2::{ enums::{ CharacterData, TemplateNames }, shared::TemplateStore };
...
impl Adapter for Gw2ApiAdapter {
    ...
    fn start(
        ...
        let template_store = cx
            ...
            .ok_or("TemplateStore extension not found")?
            .clone();
        ...
        | {
            ...
            template_store.retain(|name, _| {
                if new_map.contains_key(name) {
                    ...
                    removed += 1;
                    bus.action_notify_all(
                        "gw2-api.character-changed".to_string(),
                        Some(serde_json::json!({ "name": name, "change": "removed" }))
                    );
                    false
                }
            });
            ...
            for (name, new_t) in &new_map {
                match last_seen.get(name) {
                    None => {
                        ...
                        bus.action_notify_all(
                            "gw2-api.character-changed".to_string(),
                            Some(serde_json::json!({ "name": name, "change": "added" }))
                        );
                    ...
                    Some(old_t) => {
                        if old_t != new_t {
                            changed += 1;
                            bus.action_notify_all(
                                "gw2-api.template-changed".to_string(),
                                Some(
                                    serde_json::json!({
                        "name": name,
                        "before": old_t,
        ...
        let join = thread::spawn(move || {
            ...
            loop {
                select! {
                    recv(inbox) -> msg => {
                        match msg {
                            Ok(AdapterNotify { topic, ..}) => {
                                if topic == "gw2.api.get_characters" {
